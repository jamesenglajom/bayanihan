...
const DATA = {
  header: {
    ...
    description: "Stay informed about the stories and milestones that matter to our community.",
  },
  newsList: [
    {
      badge: "Culture",
    ...
    {DATA.newsList.map((news, i) => (
      ...
      >
        <div className="relative h-56 overflow-hidden">
          <img src={news.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={news.title} />
          <div className="absolute top-4 left-4 bg-theme-blue text-white text-xs font-bold px-3 py-1 rounded-full">{news.badge}</div>
        </div>
        <div className="p-6">
          <p className="text-theme-red text-xs font-bold mb-2 tracking-widest uppercase">{news.read_duration}</p>
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2">{news.excerpt}</p>
...
const Version2Spotlight = () => (
  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
    <motion.div variants={FADE_UP} className="lg:col-span-7 group relative rounded-3xl overflow-hidden h-[500px]">
      <img src={DATA.newsList[0].image} className="absolute inset-0 w-full h-full object-cover" alt="Featured" />
      ...
      <div className="absolute bottom-0 p-8 lg:p-12">
        <span className="bg-theme-yellow text-theme-dark px-4 py-1 rounded-md font-bold text-xs mb-4 inline-block">{DATA.newsList[0].badge}</span>
        <h3 className="text-3xl md:text-4xl font-bold text-white mb-4 leading-tight">{DATA.newsList[0].title}</h3>
        <p className="text-theme-alice/80 max-w-md mb-6">{DATA.newsList[0].excerpt}</p>
        <button className="bg-theme-blue text-white px-6 py-3 rounded-full font-bold hover:bg-theme-red transition-colors">Read Featured Story</button>
      </div>
    </motion.div>
    <div className="lg:col-span-5 flex flex-col gap-6">
      {DATA.newsList.slice(1).map((news, i) => (
        <motion.div key={i} variants={FADE_UP} className="flex gap-4 p-4 rounded-2xl bg-theme-alice/30 dark:bg-theme-dark border border-transparent hover:border-theme-blue/30 transition-all">
          <img src={news.image} className="w-24 h-24 rounded-xl object-cover" alt="" />
          <div>
            <span className="text-theme-red font-bold text-[10px] uppercase">{news.badge}</span>
            <h4 className="font-bold text-theme-dark dark:text-theme-alice line-clamp-2 group-hover:text-theme-blue transition-colors">{news.title}</h4>
...
const Version3List = () => (
  <div className="max-w-4xl mx-auto divide-y divide-gray-200 dark:divide-theme-alice/10">
    {DATA.newsList.map((news, i) => (
      <motion.div key={i} variants={FADE_UP} className="py-8 group flex flex-col md:flex-row md:items-center gap-6 justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-4 mb-2">
            <span className="text-theme-blue font-black text-xs tracking-tighter italic">0{i+1}</span>
            <span className="text-gray-400 text-xs">{news.date}</span>
          </div>
          <h3 className="text-2xl font-bold text-theme-dark dark:text-theme-alice group-hover:text-theme-red transition-colors">{news.title}</h3>
          <p className="text-gray-500 text-sm mt-2 line-clamp-1">{news.excerpt}</p>
        </div>
        <div className="flex items-center gap-4">
            <span className="hidden md:block text-xs font-bold text-theme-dark dark:text-theme-alice">{news.read_duration}</span>
...
const Version4Bento = () => (
  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[200px]">
    {/* Large Tile */}
    <motion.div variants={FADE_UP} className="md:col-span-2 md:row-span-2 relative rounded-3xl overflow-hidden bg-theme-blue p-8 flex flex-col justify-end group">
        <img src={DATA.newsList[1].image} className="absolute inset-0 opacity-40 object-cover w-full h-full grayscale group-hover:grayscale-0 transition-all duration-700" alt="" />
        <div className="relative z-10">
            <h3 className="text-3xl font-bold text-white mb-2">{DATA.newsList[1].title}</h3>
            <p className="text-theme-cream text-sm">Latest Outreach Milestone</p>
        </div>
    </motion.div>
    {/* Side Tiles */}
    {DATA.newsList.map((news, i) => (
        <motion.div key={i} variants={FADE_UP} className={`md:col-span-2 rounded-3xl p-6 flex flex-col justify-center border-2 border-theme-alice dark:border-theme-dark/50 bg-white dark:bg-theme-dark/40`}>
            <div className="flex justify-between items-start">
                <span className="w-2 h-2 rounded-full bg-theme-red animate-pulse"></span>
                <span className="text-[10px] font-bold text-theme-blue uppercase tracking-widest">{news.badge}</span>
            </div>
            <h4 className="mt-4 font-bold text-theme-dark dark:text-theme-alice text-lg leading-tight">{news.title}</h4>
        </motion.div>
    ))}
  </div>
);
  ...
  return (
    <section id="news" className="w-full py-20 bg-theme-alice dark:bg-theme-dark transition-colors duration-500 overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        {/* REUSABLE HEADER */}
        <SiteSectionHeader tagline={DATA.header.tagline} title={DATA.header.title} description={DATA.header.description} />
        

        {/* VERSIONS TOGGLE: Simply comment/uncomment the one you want to preview */}
        <motion.div initial="hidden" whileInView="visible" variants={STAGGER_CONTAINER} viewport={{ once: true }}>
