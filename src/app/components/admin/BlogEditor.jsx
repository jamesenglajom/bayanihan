    ...
    <div className="flex items-start gap-5 p-6 mb-10 bg-indigo-50/40 border border-indigo-100/50 rounded-[2rem] shadow-sm backdrop-blur-sm">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-sm text-indigo-800/80 leading-relaxed font-medium">
          ...
          <strong className="text-indigo-900 font-black">BES Library</strong>{" "}
...
const BlogEditor = ({ blog }) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-slate max-w-none focus:outline-none min-h-[600px] p-8 md:p-16 text-slate-700 leading-relaxed text-lg",
      },
    },
  });
  ...
  const labelStyle =
    "text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 flex items-center gap-2 mb-2 px-1";
  const inputStyle =
        ...
        <div className="flex items-center gap-4">
          {blog && (
            ...
            >
              <ExternalLink
                size={12}
                className="text-indigo-500 group-hover/link:text-white transition-colors"
              />
              View Live Story
            </a>
          )}
        ...
        <div className="lg:col-span-8 space-y-10">
          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden group/card">
              ...
              {post.main_image ? (
                <>
                  ...
                  <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/card:opacity-100 transition-all duration-300 flex items-center justify-center gap-3 backdrop-blur-sm">
                    <div className="px-6 py-3 bg-white text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl">
                    ...
                    <button
                      ...
                      className="p-3 bg-rose-500 text-white rounded-2xl hover:bg-rose-600 shadow-xl transition-all"
                    >
                      <X size={20} />
                    </button>
                  </div>
            ...
            <div className="p-10 md:p-14">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-500 block mb-6">
                Headline
              </label>
              <textarea
            ...
            {editor && (
              <div className="sticky top-6 z-40 mx-auto max-w-fit px-5 py-2.5 bg-slate-900/90 backdrop-blur-xl rounded-2xl flex items-center gap-1 shadow-2xl shadow-slate-900/20 border border-slate-800">
                <button
                  ...
                  className={`p-2.5 rounded-xl transition-all ${editor.isActive("bold") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/40" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
                >
                  <Bold size={16} />
                </button>
                <button
                  onClick={() => editor.chain().focus().toggleItalic().run()}
                  className={`p-2.5 rounded-xl transition-all ${editor.isActive("italic") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/40" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
                >
                  <Italic size={16} />
                </button>
                <button
                  onClick={() => editor.chain().focus().toggleUnderline().run()}
                  className={`p-2.5 rounded-xl transition-all ${editor.isActive("underline") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/40" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-[10px] font-black transition-all ${editor.isActive("heading", { level: 2 }) ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/40" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
                >
                  H2
                </button>
                <button
                  ...
                  className={`p-2.5 rounded-xl transition-all ${editor.isActive("bulletList") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/40" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
                >
                  <List size={16} />
                </button>
                <div className="w-px h-5 bg-slate-700 mx-2" />
                <button
                  ...
                  className="p-2.5 rounded-xl text-slate-400 hover:bg-indigo-600 hover:text-white transition-all flex items-center gap-2 group/btn"
                >
                  <ImageIcon size={16} />{" "}
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-10">
            ...
            <div className="space-y-1">
              <label className={labelStyle}>
                <Calendar size={14} className="text-indigo-500" /> Publication
                Date
              </label>
              <input
                type="date"
                ...
                className={`${inputStyle} cursor-pointer [color-scheme:light]`}
              />
            </div>

            <div className="space-y-1">
              <label className={labelStyle}>
                <User size={14} className="text-indigo-500" /> Lead Author
              </label>
              <input
                type="text"
                value={post.author}
            ...
            <div className="space-y-1">
              <label className={labelStyle}>
                <Tag size={14} className="text-indigo-500" /> Display Category
              </label>
              <input
                type="text"
                value={post.badge}
            ...
            <div className="space-y-1">
              <label className={labelStyle}>
                <FileText size={14} className="text-indigo-500" /> SEO Metadata
              </label>
              <textarea
                value={post.excerpt}
            ...
            <div className="space-y-4">
              <label className={labelStyle}>
                <Hash size={14} className="text-indigo-500" /> Semantic Tags
              </label>
              <div className="flex flex-wrap gap-2">
                {post.categories.map((cat, i) => (
                  <span
                    key={i}
                    className="flex items-center gap-2 px-3 py-2 bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-slate-200"
                  >
                    {cat}{" "}
                    <X
                      size={12}
